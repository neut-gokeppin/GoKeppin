	...
	public void okButtonMain()
	{
		...
		try {
			...
			if ((checkList !=null) && (checkList.size() > 0)) {
				...
				// 2016/07/15 DA del end
				// 2016/07/15 DA ins start
				isNoXmlFile = Boolean.valueOf(map.get("noXmlFile"));
				isNotPassing = Boolean.valueOf(map.get("notPassing"));
				isExpired = Boolean.valueOf(map.get("expired"));
				// 2016/07/15 DA ins end
			}

			// 最新ラインオフ計画日取得失敗（車両取得に失敗）
...
//						searchFlg = true;
//					}
//					else if (isKensaDataReacquisition()) {
//						// 検査データ削除
//						deleteKensaData();
//
//						// 検査データ取得
...
// 2017/12/01 DA del end
		...
		if (isJudgment) {
			...
			// 撮影画像の削除
			sidown.deleteUnnecessaryShotimage(getApplicationContext());
			// 画像キャッシュをクリア
			ResizedShotimageCache.clearImage();
			// 2018/09/25 DA ins end

			// 撮影画像取得に移る
			downloadShotimage(sql);
		}
		else {
			sidown.setTacttime(-1);
	...
	/**
	 * 撮影画像の取得処理
	 * @param sql 検査データ取得SQL
	 */
	private void downloadShotimage(final String sql)
	{
		// 待機メッセージを変える
		handler.post(new Runnable()
		{
			public void run()
			{
				progressDialog.setMessage(Common.MSG_SHOTIMAGE_RETRIEVING);
				progressDialog.show();
			}
		});

		int isImageResult = sidown.downloadShotimage(getApplicationContext(), strGroupName, inputDataCase, LFA.mode);

		if (isImageResult == ShotimageDownload.NG) {
			// 異常はエラーメッセージを表示する
			endProcess();
			progressDialog.dismiss();
		...
		else if (isImageResult == ShotimageDownload.TIMEOUT) {
			// タイムアウトの場合、再取得の確認ダイアログを表示する
			handler.post(new Runnable()
			{
				public void run()
				{
			...
			// すぐに画面遷移するとダウンロードタイミングにより画像が表示されないことがあるため、少し待つ
			Utils.sleep(1000);
	 ...
	 * @param sql 検査データ取得SQL
	 */
	private void showRetryDialog(final String sql)
	{
		AlertDialog.Builder b = new AlertDialog.Builder(this);
		b.setTitle(getString(R.string.dialogInfoTitle)); // 確認
		b.setMessage(Common.MSG_SHOTIMAGE_TIMEOUT);
		b.setPositiveButton(getString(R.string.dialogInfoPositiveButton), new DialogInterface.OnClickListener()
		{
			// ボタンが押されたら再取得する
		...
		b.setNegativeButton(getString(R.string.dialogInfoNegativeButton), new DialogInterface.OnClickListener()
		{
			// ボタンが押されたらダイアログを閉じる
	 ...
	 * @param sql 検査データ取得SQL
	 */
	private void nextGamen(String sql)
	{
		Cursor c = null;
		try {
			// 検査データ取得は再検索
	...
	private void deleteKensaData() {
		...
		try {
			// P_ordersignHistory は無いため処理は不要。※ある場合は検査中のため削除対象とはならないです。
...
//	private boolean isKensaDataReacquisition()
//	{
//		Cursor oic = null;
//		String sql = "";
//
//		// TP通過時刻がない場合、通過されているかもしれないので再取得する
//		sql = "SELECT * FROM P_ordersingItem WHERE bodyNo='" + inputDataCase + "'";
//		oic = LFA.getmDb().rawQuery(sql, null);
//		String strTp = null;
//		if (oic.moveToFirst()) {
		...
		if (bytetest == null) {
			...
			if (byteArray == null) {
				...
				if(strResult.equals("NG")) {
					...
					try {
						// 2016/02/24 DA ins start
						String sql = "SELECT * FROM P_ordersing WHERE bodyNo = '" + inputDataCase
								+ "' AND groupCode = '" + strGroupCode + "'";
